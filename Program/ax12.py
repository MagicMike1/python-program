...
class Ax12:
    ...
    RIGTH = 1
    RX_TIME_OUT = 10
    #TX_DELAY_TIME = 0.000085
    TX_DELAY_TIME = 0.00005
    #TX_DELAY_TIME = 0.000084
    TX_DELAY_TIME_READ = 0.00005
    TX_DELAY_TIME_WRITE = 0.00041
    ...
    def __init__(self):
        if(Ax12.port == None):
            Ax12.port = Serial("/dev/ttyS0", baudrate=1000000, timeout=0.012)
        if(not Ax12.gpioSet):
    ...
    def direction(self,d):
        GPIO.output(int(Ax12.RPI_DIRECTION_PIN), int(d))
        sleep(Ax12.RPI_DIRECTION_SWITCH_DELAY)

    def readData(self,id):
        GPIO.output(int(Ax12.RPI_DIRECTION_PIN), int(GPIO.LOW))
        sleep(Ax12.RPI_DIRECTION_SWITCH_DELAY)
        ...
        try :
            ...
            if(error != 0):
                return ("Error from servo: " + Ax12.dictErrors[error] + ' (code  ' + hex(error) + ')')
                return -error
    ...
    def move(self, id, position):
        ...
        outData += chr(checksum)
        Ax12.port.write(outData)
        sleep(Ax12.TX_DELAY_TIME_WRITE)
        return self.readData(id)
    ...
    def learnServos(self,minValue=1, maxValue=18, verbose=True) :
        servoList = []
