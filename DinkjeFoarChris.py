...
def move(leg, pos1, pos2, pos3, time):
    ...
    print(servo.readTemperature(legs[leg][0]), servo.readTemperature(legs[leg][1]), servo.readTemperature(legs[leg][2]))
    print(servo.readPosition(legs[leg][0]), servo.readPosition(legs[leg][1]), servo.readPosition(legs[leg][2]))
...
def walk():
    ...
    for x in range (0, 6):
        ...
        if(leg6 > 5):
            leg6 = 0
        servo.action()
        sleep(0.12) #0.02 sleep for every 0.1 speed
